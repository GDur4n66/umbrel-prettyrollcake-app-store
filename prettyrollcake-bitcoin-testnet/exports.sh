...
if [[ ! -f "${BITCOIN_ENV_FILE}" ]]; then
	...
	echo "export APP_BITCOIN_TESTNET_NETWORK='${BITCOIN_NETWORK}'"		>  "${BITCOIN_ENV_FILE}"
	echo "export APP_BITCOIN_TESTNET_RPC_USER='${BITCOIN_RPC_USER}'"	>> "${BITCOIN_ENV_FILE}"
	echo "export APP_BITCOIN_TESTNET_RPC_PASS='${BITCOIN_RPC_PASS}'"	>> "${BITCOIN_ENV_FILE}"
	echo "export APP_BITCOIN_TESTNET_RPC_AUTH='${BITCOIN_RPC_AUTH}'"	>> "${BITCOIN_ENV_FILE}"
fi

. "${BITCOIN_ENV_FILE}"

if [[ "${APP_BITCOIN_TESTNET_NETWORK}" == "mainnet" ]]; then
	BITCOIN_CHAIN="main"
elif [[ "${APP_BITCOIN_TESTNET_NETWORK}" == "testnet" ]]; then
	BITCOIN_CHAIN="test"
	export APP_BITCOIN_TESTNET_RPC_PORT="18332"
	export APP_BITCOIN_TESTNET_P2P_PORT="18333"
	export APP_BITCOIN_TESTNET_TOR_PORT="18334"
elif [[ "${APP_BITCOIN_TESTNET_NETWORK}" == "signet" ]]; then
	BITCOIN_CHAIN="signet"
	export APP_BITCOIN_TESTNET_RPC_PORT="38332"
	export APP_BITCOIN_TESTNET_P2P_PORT="38333"
	export APP_BITCOIN_TESTNET_TOR_PORT="38334"
elif [[ "${APP_BITCOIN_TESTNET_NETWORK}" == "regtest" ]]; then
	BITCOIN_CHAIN="regtest"
	export APP_BITCOIN_TESTNET_RPC_PORT="18443"
	export APP_BITCOIN_TESTNET_P2P_PORT="18444"
	export APP_BITCOIN_TESTNET_TOR_PORT="18445"
else
	echo "Warning (${EXPORTS_APP_ID}): Bitcoin Network '${APP_BITCOIN_TESTNET_NETWORK}' is not supported"
fi
...
if [[ "${APP_BITCOIN_TESTNET_NETWORK_ELECTRS}" = "mainnet" ]]; then
	APP_BITCOIN_TESTNET_NETWORK_ELECTRS="bitcoin"
fi
